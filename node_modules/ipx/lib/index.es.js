import { resolve, posix, relative, dirname, extname } from 'path';
...
const consola = consola2.withTag("ipx");
...
function getConfig(options) {
  const config = defu2(options, defaults);
...
class RemoteAdapter extends BaseInputAdapter {
  ...
  async stats(src) {
    ...
    try {
      const stats = await stat(_src.cache);
      return stats;
    } catch (e) {
      return false;
    }
...
class FSCache extends BaseCacheAdapter {
  ...
  async clean() {
    ...
    await Promise.all(items.map(async (item) => {
      consola2.debug("[DELETE] " + item.file);
...
class IPX {
  ...
  init() {
    ...
    if (typeof this.options.cache.adapter === "string") {
      ...
      CacheCtor = CacheAdapters[adapter] || require(resolve(adapter));
    } else {
    ...
    if (this.options.cache.cleanCron) {
      this.cacheCleanCron = new CronJob(this.options.cache.cleanCron, () => this.cleanCache().catch(consola.error));
  ...
  initInputs(inputs) {
    this.inputs = inputs.map(({adapter, ...options}) => {
      ...
      if (typeof adapter === "string") {
        InputCtor = InputAdapters[adapter] || require(resolve(adapter));
      } else {
        InputCtor = adapter;
      }
...
async function IPXReqHandler(req, res, ipx2) {
  ...
  const etag2 = getEtag(info.cacheKey);
